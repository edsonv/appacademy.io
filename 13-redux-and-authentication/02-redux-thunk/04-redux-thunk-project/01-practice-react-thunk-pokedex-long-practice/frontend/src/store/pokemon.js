import { ADD_ITEM, LOAD_ITEMS, REMOVE_ITEM } from "./items";
...
export const createPokemon = (pokemonData) => async (dispatch) => {
  console.log("createPokemon payload:", pokemonData);
  const response = await fetch("/api/pokemon", {
    method: "POST",
    headers: {
...
export const updatePokemon = (pokemonData) => async (dispatch) => {
  const response = fetch(`/api/pokemon/${pokemonData.id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
